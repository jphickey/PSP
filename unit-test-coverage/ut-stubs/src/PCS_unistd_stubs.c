...
int PCS_gethostname(char *name, size_t len)
{
    ...
    UT_GenStub_Execute(PCS_gethostname, Basic, NULL);
...
PCS_ssize_t PCS_read(int fd, void *buf, size_t nbytes)
{
    ...
    UT_GenStub_Execute(PCS_read, Basic, NULL);
...
PCS_ssize_t PCS_write(int fd, const void *buf, size_t n)
{
    ...
    UT_GenStub_AddParam(PCS_write, size_t, n);

    UT_GenStub_Execute(PCS_write, Basic, NULL);
